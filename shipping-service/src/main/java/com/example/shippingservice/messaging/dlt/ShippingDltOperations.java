...
@Slf4j
public class ShippingDltOperations {

    private static final List<String> DLT_TOPICS = List.of("shipment-requested.DLT", "order-cancelled.DLT");
    ...
    public void replayShipmentRequested(ShipmentRequestedEvent event) {
        send("shipment-requested", event.getOrderId(), event);
    }

    public void replayOrderCancelled(OrderCancelledEvent event) {
        send("order-cancelled", event.getOrderId(), event);
    }
