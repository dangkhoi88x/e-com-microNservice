...
public class FlashDealController {
    ...
    @PostMapping @PreAuthorize("hasAuthority('ROLE_ADMIN')") public ApiResponse<FlashDealResponse> create(@Valid @RequestBody CreateFlashDealRequest request) { return body(HttpStatus.CREATED, service.create(request)); }
    @GetMapping @PreAuthorize("hasAuthority('ROLE_ADMIN')") public ApiResponse<List<FlashDealResponse>> getAll(@RequestParam(required = false) String status) { return body(HttpStatus.OK, service.getAll(status)); }
    ...
    @PutMapping("/{id}") @PreAuthorize("hasAuthority('ROLE_ADMIN')") public ApiResponse<FlashDealResponse> update(@PathVariable String id, @Valid @RequestBody CreateFlashDealRequest request) { return body(HttpStatus.OK, service.update(id, request)); }
    @DeleteMapping("/{id}") @PreAuthorize("hasAuthority('ROLE_ADMIN')") public ApiResponse<Void> delete(@PathVariable String id) { service.delete(id); return body(HttpStatus.OK, null); }
