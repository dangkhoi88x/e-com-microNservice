...
public class WishlistServiceImpl implements WishlistService {
    ...
    @Override public WishlistItemResponse addItem(String userId, AddWishlistItemRequest request) { return repository.findByUserIdAndProductIdAndVariantId(userId, request.productId(), request.variantId()).map(this::response).orElseGet(() -> response(repository.save(WishlistItem.builder().userId(userId).productId(request.productId()).variantId(request.variantId()).productName(request.productName()).price(request.price()).imageUrl(request.imageUrl()).categoryName(request.categoryName()).build()))); }
