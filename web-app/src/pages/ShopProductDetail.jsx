...
const productUrl = (item) =>
  `/shop/products/${slugify(item.name)}-${item.productId || item.id}`;
...
export default function ShopProductDetail() {
  ...
  useEffect(() => {
    searchProducts({ page: 1, size: 60, inStock: true, sort: "createdAt,desc" })
      .then((data) => setCatalogProducts(data.content || []))
      .catch(() => {});
  ...
  useEffect(() => {
    ...
    const refreshCart = () =>
      getMyCart()
        .then((data) => setCartCount(cartQuantity(data)))
  ...
  if (!product)
    return <main className="detail-loading">Đang tải sản phẩm…</main>;
  ...
  return (
    <main className="product-detail-page">
      <header className="detail-top detail-store-header">
        <Link to="/shop">
          <ShoppingBagOutlinedIcon /> NovaShop
        </Link>
        ...
        <div className="detail-header-actions">
          <Link className="detail-cart-link" to="/cart">
            <AddShoppingCartOutlinedIcon /> Giỏ hàng{" "}
            {cartCount > 0 && <b>{cartCount}</b>}
          </Link>
          <button onClick={() => navigate(-1)}>
            <ArrowBackOutlinedIcon /> Quay lại
          </button>
        </div>
      </header>
      <nav className="detail-breadcrumb" aria-label="Breadcrumb">
        ...
        {product.categoryId ? (
          <Link
            to={`/shop/categories/${slugify(product.categoryName)}-${product.categoryId}`}
          >
      ...
      <section className="detail-layout">
        <aside className="detail-options">
          ...
          {(product.options || []).map((option) => (
            <section key={option.name}>
              ...
              <div
                className={`detail-option-values ${option.displayType === "COLOR_SWATCH" ? "colors" : ""}`}
              >
                {option.values
                  ...
                  .map((value) => (
                    ...
                    >
                      {option.displayType === "COLOR_SWATCH" ? (
                        <span
                          style={{ background: value.colorHex || "#a9d6e8" }}
                        />
                      ) : (
                        value.displayValue
                      )}
        ...
        <section className="detail-info">
          <p>{product.categoryName || "Sản phẩm"}</p>
          <h1>{product.name}</h1>
          <div className="detail-rating">
            <StarRoundedIcon /> {Number(reviewSummary.averageRating || 0).toFixed(1)} <span>({reviewSummary.reviewCount || 0} đánh giá)</span>
          </div>
                  ...
                  {Math.round(
                    Number(
                      activeSale.discountPercent ||
                        100 - (Number(price) * 100) / Number(originalPrice),
                    ),
                  )}
                  % ·{" "}
                  {activeSale.saleType === "LONG_TERM"
                    ? "SALE DÀI HẠN"
                    : "FLASH SALE"}
                </b>
              </>
          ...
          <div className="detail-actions">
            ...
            <button className="detail-add" disabled={!stock} onClick={add}>
              <AddShoppingCartOutlinedIcon /> Thêm giỏ
            </button>
            <button className="detail-buy" disabled={!stock} onClick={buyNow}>
              Mua ngay
            </button>
          </div>
          {cartNotice && <p className="detail-cart-notice">{cartNotice}</p>}
          <small className="detail-delivery">
            <LocalShippingOutlinedIcon /> Miễn phí vận chuyển cho đơn đủ điều
            kiện
          </small>
          <button
            className={`detail-wishlist ${wished ? "is-wishlisted" : ""}`}
            disabled={wishlistLoading || wishlistPending}
          ...
          {wishlistNotice && (
            <p className="detail-wishlist-notice">{wishlistNotice}</p>
          )}
        </section>
      </section>
      <section className="detail-content-sections">
        <article className="detail-specifications">
          <h2>Product Specifications</h2>
          <dl>
            <div>
              <dt>Danh mục</dt>
              <dd>{product.categoryName || "Đang cập nhật"}</dd>
            </div>
            <div>
              <dt>SKU</dt>
              <dd>{variant?.sku || product.id}</dd>
            </div>
            <div>
              <dt>Tình trạng</dt>
              <dd className={stock > 0 ? "in-stock" : "out-stock"}>
            ...
            {Object.entries(variant?.attributes || selected).map(
              ([key, value]) => (
                <div key={key}>
                  <dt>{key}</dt>
                  <dd>{value}</dd>
        ...
        <article className="detail-product-description">
          <h2>Product Description</h2>
          <p>
            {product.description ||
              "Sản phẩm được tuyển chọn với thiết kế hiện đại, chất lượng tốt và phù hợp cho nhu cầu sử dụng hằng ngày."}
          </p>
        </article>
      </section>
      <ProductReviews product={product} onSummaryChange={setReviewSummary} />
      <section className="detail-related">
        <h2>Sản phẩm liên quan</h2>
        <div>
          {related.map((item) => (
            <article key={item.id} onClick={() => navigate(productUrl(item))}>
              <span>
                {firstImage(item) ? (
